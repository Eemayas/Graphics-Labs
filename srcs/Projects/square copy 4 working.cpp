...
void drawCircle(float centerX, float centerY, float radius, float speed, int loop, GLfloat size, float *i)
{
    ...
    *i += (1.0f / speed);
    ...
    {
        std::cout << "ggg" << ggg << "\n";
        outerPoints.push_back(std::make_pair(1100 + ggg * WINDOW_WIDTH, circlePoints[second_circle].second));
        ggg = ggg + 1.0 / WINDOW_WIDTH;
    }
    if (loop == 0)
    {
        glPointSize(1);
        glBegin(GL_POINTS);
        int gg = 0;
        for (auto &coord : outerPoints)
        {
            std::cout << "(" << coord.first << ", " << coord.second << ")\n";
            // std::cout << "(" << (float)(1100 + gg) << ", " << coord.second << ")\n";
