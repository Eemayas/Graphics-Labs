...
void drawRectangle(float *color, float centerX, float centerY, float width, float height)
{
    // Set color
    glColor4fv(color);
...
void getTheScreenResolution()
{
    // Get the primary monitor
    GLFWmonitor *monitor = glfwGetPrimaryMonitor();

    // Get the video mode of the primary monitor
    const GLFWvidmode *screenResolution = glfwGetVideoMode(monitor);

    // Print the screen resolution
    std::cout << "Screen Resolution: " << screenResolution->width << "x" << screenResolution->height << std::endl;
}
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Call drawRectangle function to draw the rectangle with  given coordinates and colora
        drawRectangle(blueish, centerX, centerY, WINDOW_WIDTH * 0.8, WINDOW_HEIGHT * 0.8);
