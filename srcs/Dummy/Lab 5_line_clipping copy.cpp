...
void cohenSutherlandLineClipAndDraw(float x1, float y1, float x2, float y2)
{
    ...
    if (accept)
    {
        ...
        clippedLineCoordinate[1].second = y1;
        std::cout << "The line is accepted and corrdinate is: {" << x1 << "," << y1 << "} , {" << x2 << "," << y2 << "}" << std::endl;
        // Draw the line
        glBegin(GL_LINES);
        glVertex2d(x1, y1);
        glVertex2d(x2, y2);
    ...
    {
        std::cout << "The line isnot accepted " << std::endl;
    }
}
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        cohenSutherlandLineClipAndDraw(50, 50, 800, 800);
