...
void drawOuterRedSection()
{
    ...
    glBegin(GL_POLYGON);
    glColor4fv(red); // Set the color to red
...
void drawTemple(float x, float y, float angle, int n)
{
    ...
    glBegin(GL_LINES);
    glVertex2f(-length / 2 + increment / 2, -distance); // Starting point of the second line
    glVertex2f(length / 2 - increment / 2, -distance);  // Ending point of the second line
    glEnd();
    ...
    { // Draw the third line parallel to the first line and 10.0f below
        ...
        glBegin(GL_LINES);
        glVertex2f(-length / 2, 0); // Starting point of the third line
        glVertex2f(+length / 2, 0); // Ending point of the third line
        glEnd();
        glBegin(GL_LINES);
        glVertex2f(-length / 2, +distance); // Starting point of the third line
        glVertex2f(+length / 2, +distance); // Ending point of the third line
        glEnd();
    }
    if ((n == 3))
    {
        glBegin(GL_LINES);
        glVertex2f(-length, distance); // Starting point of the third line
        glVertex2f(+length, distance); // Ending point of the third line
        glEnd();
        glBegin(GL_LINES);
        glVertex2f(-length, distance * 2); // Starting point of the third line
        glVertex2f(+length, distance * 2); // Ending point of the third line
        glEnd();
    }
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        drawLinesPerpendicularToCircle(WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2, 300, MAX_SEGMENTS);

        // Call drawJoinedLines function to draw the joined lines
        drawOuterRedSection();

        // Call drawCircle function to draw circle after red and fill a circle with skyBlue
        drawCircle(WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2, 300, 100, skyBlue);

        // Call drawCircle function to draw circle after skyBlue and fill a circle with yellow
